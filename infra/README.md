...
- `preprod/`, `prod/` — per-environment Docker Compose stacks (Postgres + backend + frontend). Each joins the VPS's external `edge` network and gets routed via Docker labels, no Caddyfile to keep in sync (see `docs/adr/0010-*.md`).
