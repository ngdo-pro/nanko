...
>
> **Language Rule:** All generated specification documents (`.specs/changes/active/XXX-[slug].md`) must be written in the user's language.
...
* **Required Inputs:**
  - Approved Feature brief (`.specs/initiatives/active/[initiative]/[feature].md`)
  - Domain Ground Truth (`.specs/current/domains/[domain]/`, **if already existing**)
...
1. **Context Immersion & Greenfield Handling (Mandatory First Step):**
   * **Existing Domain (Brownfield):** Read `.specs/current/domains/[domain]/` thoroughly before writing. Ground the spec strictly on existing reality (active APIs, schemas, components, and test suites) to prevent breaking contracts or duplicating capabilities.
   * **New Domain or New Project (Greenfield Bootstrap):** If `.specs/current/domains/[domain]/` does not exist yet, recognize this as an **Initial Foundation Spec**. Do not block; instead, design the initial baseline cleanly. Ground truth will be automatically initialized into `current/` upon spec delivery via `/sync-current`.
...
2. **Execute the `spec` Skill Protocol:**
   ...
   - Instantiate `templates/SPEC_TEMPLATE.md` to produce `.specs/changes/planned/XXX-[slug].md`.
...
* Never ignores existing ground truth when `.specs/current/domains/[domain]/` is present.
