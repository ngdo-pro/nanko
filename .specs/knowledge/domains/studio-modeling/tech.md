...
| **`INV-TECH-07`** | **Distribution Multi-Ports & Auto-Scale Décongestionné** : Pour toute face comptant $N$ connecteurs, les poignées sont réparties à $\frac{1}{N+1}\dots$ ordonnées selon la position opposée. Le seuil de 8 flux déclenche l'échelle $\times 2$ intégrée dans Dagre | Module `anchorDistribution.ts`, `calculateDagreLayout` |
