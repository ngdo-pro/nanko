...
| **`INV-TECH-09`** | **Repositionnement Manuel des Labels & Persistance Différentielle** : Glisser interactif avec capture de pointeur (`setPointerCapture`), calcul d'amplitude compensé par le zoom viewport (`/ currentZoom`), seuil de déplacement (3px) et double-clic de purge. Synchronisation différentielle dans `syncLayoutToSource.ts` supportant l'insertion, la modification et le nettoyage propre des clés `labelX` et `labelY` sans altérer les directives `from`/`to` existantes | Modules `NankoEdge.tsx`, `syncLayoutToSource.ts`, `NankoParser.php`, `NankoAst.php` |
