...
| **`INV-TECH-10`** | **Création Rapide Connectée par Dépôt dans le Vide (Miro Quick Spawn) & Mutation Atomique Unique** : Détection dans `onConnectEnd` du lâcher dans un espace libre (`!closest('.react-flow__node') && !closest('.react-flow__handle')`). Affichage du menu contextuel `QuickSpawnMenu` sous le curseur avec capture de raccourcis (`R`, `C`, `Escape`). Calcul de centrage géométrique de la nouvelle forme (`calculateCenteredShapePosition`). Mutation unifiée (`quickSpawnConnectedShape.ts`) combinant en une seule passe `insertShapeToSource` et `insertConnectorToSource` avec ancre cible opposée canonique (`getOppositeAnchorSide`). Annulation propre par Escape ou clic externe sans modification textuelle | Modules `quickSpawnConnectedShape.ts`, `QuickSpawnMenu.tsx`, `NankoCanvas.tsx`, `DocumentEditorView.tsx` |
...
| [`Spec 026`](../../specs/archive/026-connector-quick-spawn-on-drop.md) | Création Rapide Connectée par Dépôt dans le Vide (Miro Quick Spawn) | Détection lâcher hors cible `onConnectEnd`, composant `QuickSpawnMenu`, mutation atomique unique DSL `quickSpawnConnectedShape` |
