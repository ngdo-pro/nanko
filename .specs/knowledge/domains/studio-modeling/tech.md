...
| **`INV-TECH-07`** | **Distribution Multi-Ports & Auto-Scale Décongestionné** : Pour toute face comptant $N$ connecteurs, les points d'attache passifs (`isConnectable={false}`) sont réparties à $\frac{1}{N+1}\dots$ ordonnées selon la position opposée. Seules les 5 poignées canoniques sont connectables. L'insertion via la roue radiale est atomique hors `setState`. Le seuil de 8 flux déclenche l'échelle $\times 2$ intégrée dans Dagre | Module `anchorDistribution.ts`, `NankoCanvas.tsx`, `calculateDagreLayout` |
