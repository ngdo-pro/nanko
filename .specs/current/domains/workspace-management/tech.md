...
* **Module Documents (`features/documents/`) :**
  ...
  * `components/` : `CreateDocumentModal`, `DocumentCard`, `DocumentList`, `SourceCodeEditor`, `AstInspector` (badge `DSL v1`, affichage de `label` et `desc`).
    * `components/canvas/` : `NankoCanvas` (composant React Flow principal avec gestion dark/light et minimap, injection de `desc` dans `rawNodes` et `rawEdges`, capture des raccourcis clavier `A`, `Tab`, `R`, `C` avec protection contre le focus d'inputs, projection spatiale `screenToFlowPosition`), `LayoutSelector` (sélecteur Split / Canvas / Code), `CanvasControls` (barre flottante dans `<Panel position="bottom-left">`), `nodes/` (`RectangleNode`, `CircleNode` avec vraie géométrie circulaire 130px × 130px, `TextNode` avec affichage du sous-titre `.nanko-node-desc` tronqué à 2 lignes et infobulle Blueprint — styles scopés en CSS Modules `*.module.css`), `edges/` (`NankoEdge` avec badge de label épuré, puce indicatrice et infobulle — scopé en `NankoEdge.module.css`).
    * `components/canvas/tooltip/` : `BlueprintTooltip` (infobulle technique Blueprint avec typographie adaptative dark/light, fond translucide avec flou, flèche indicatrice, indicateur d'ancrage/défilement et isolation d'événements React Flow `nodrag`, `nowheel`, `nopan`), `useHoverTooltip.ts` (hook de gestion des délais d'apparition 300 ms et de grâce 250 ms).
...
* **Mise en Page & Vue Éditeur (`components/layout/AppLayout.tsx` & `views/DocumentEditorView.tsx`) :**
  ...
  * Design System Blueprint : Tokens `--canvas-bg` dédié (`#011C25` en Dark Mode conforme au mockup vert nuit pétrole, `#FFFFFF` en Light Mode) avec trame orthogonale de 26px (`BackgroundVariant.Lines`), tokens sémantiques complets (`--danger`, `--success`, `--warning` light/dark), styles de sous-titres tronqués (`.nanko-node-desc`) et tooltips Blueprint adaptatifs dark/light (`.nanko-blueprint-tooltip`). Standard de styles composants : CSS Modules colocalisés (100% des composants applicatifs migrés) avec conservation des sélecteurs legacy pour rétrocompatibilité des tests.
...
* **Visualisation Canvas React Flow & Disposition Studio (`tests/app/canvas-visualization.spec.ts`) :**
  ...
  * Validation du sous-titre de description tronqué à 2 lignes avec ellipse.
  * Validation du déclenchement de l'infobulle Blueprint au survol d'un nœud et d'un badge de connecteur révélant la description intégrale.
...
* `ADR-0020` : Standardisation CSS Frontend par CSS Modules Colocalisés & Design Tokens Natifs.
